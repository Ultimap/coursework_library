...
async def get_status_book(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_session)):
    if user.role == 1:
        books = await db.execute(select(Accounting))
        return books.scalars().all()
    raise HTTPException(status_code=403)


@accounting.get('/users', status_code=200)
async def get_users(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_session)):
    if user.role == 1:
        user_book = await db.execute(select(UserBook))
